        ...
        .get(
          {
            ...
            port: 443,
            path: `/otn/leftTicket/queryA?leftTicketDTO.train_date=${options.train_date}&leftTicketDTO.from_station=${options.from_station}&leftTicketDTO.to_station=${options.to_station}&purpose_codes=${options.purpose_codes}`,
            method: 'GET',
            headers: {
              Host: 'kyfw.12306.cn',
            ...
            res.on('end', () => {
              ...
              if (data) {
                ...
                if (json.httpstatus === 200) {
                  result = json.data.result.map((l: string) => {
                    let s = l.split('|');
                    return {
                      train_secret: s[0],
                      train_state: s[1],
